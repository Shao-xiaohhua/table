...
(function($){   
  ...
  var drag = null;      //引用当前拖动的手柄
  var tables = {};      //对已经处理的表对象（table.id为重点）
  var count = 0;        //内部计数在需要时创建唯一ID.  
  ...
  var S;
  try{S = sessionStorage;}catch(e){}  //Firefox作为本地文件系统崩溃时被执行
  ...
  /**
   * 函数允许对表对象进行列大小调整。它是应用插件的起点。
   * @param {DOM node} tb - 引用要增强的DOM表对象
   * @param {Object} 选项-某些自定义值
   */
  var init = function( tb, options){  
    var t = $(tb);            //表对象已包装
        t.opt = options;                //每个表在任何时候都有自己的选项。
        t.mode = options.resizeMode;    //快捷键
        t.dc = t.opt.disabledColumns;
    if(t.opt.disable) return destroy(t);        //用户要求摧毁以前colresized表
    var id = t.id = t.attr(ID) || SIGNATURE+count++;  //如果生成新的null，则获得它的id    
    t.p = t.opt.postbackSafe;               //短切回安全检测   
    if(!t.is("table") || tables[id] && !t.opt.partialRefresh) return;     //如果对象不是一个表，或者它已经被处理，那么它将被忽略。
    if (t.opt.hoverCursor !== 'e-resize') h.append("<style type='text/css'>.JCLRgrip .JColResizer:hover{cursor:"+ t.opt.hoverCursor +"!important}</style>");  //如果hovercursor已定，追加的风格
    t.addClass(SIGNATURE).attr(ID, id).before('<div class="JCLRgrips"/>');  //添加容器容器对象。签名类强制在固定布局模式下呈现表，以防止列的最小宽度
    t.g = []; t.c = []; t.w = t.width(); t.gc = t.prev(); t.f=t.opt.fixed;  //t.c and t.g are arrays of columns and grips respectively        
    if(options.marginLeft) t.gc.css("marginLeft", options.marginLeft);    //如果表包含边距，则必须指定它。
